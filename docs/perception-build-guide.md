...
2. **For each frame: a list of person detections** from a YOLO segmentation
   model (default `yolo26l-seg.pt`, class=`person`, confidence threshold 0.15).
...
| YOLO model | `yolo26l-seg.pt` | configurable with `YOLO_MODEL_NAME`; larger YOLO26 variants may be more accurate |

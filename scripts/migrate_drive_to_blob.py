    ...
    *,
    ...
    for index, file_item in enumerate(files, start=1):
        ...
        if can_skip_progress or can_skip_existing:
            ...
            print(f"[file {index}/{len(files)}] skip {file_item.dest_path}")
            continue
        ...
        print(f"[file {index}/{len(files)}] upload {file_item.dest_path} ({_human_bytes(len(data))}, {_human_bytes(int(rate))}/s)")
